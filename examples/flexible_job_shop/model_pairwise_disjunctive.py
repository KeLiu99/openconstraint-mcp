...
Runs standalone: python model_pairwise_disjunctive.py [data.json] [seconds] [results_dir]
"""
...
literals: dict[tuple[int, int], list] = {}
...
for job_id, job in enumerate(jobs):
    for task_id, alternatives in enumerate(job):
        ...
        for alt_id, (machine, _duration) in enumerate(alternatives):
            machine_to_ops[machine].append((job_id, task_id, alt_id))

        if len(alternatives) == 1:
            model.add(end == start + alternatives[0][1])
            literals[job_id, task_id] = []
            continue

        task_literals = [
            model.new_bool_var(f"assign{suffix}_{alt_id}") for alt_id in range(len(alternatives))
        ]
        ...
        model.add(
            ...
            == start
            + sum(
                duration * task_literals[alt_id]
                for alt_id, (_machine, duration) in enumerate(alternatives)
            )
        )
        literals[job_id, task_id] = task_literals


def assignment_literal(job_id: int, task_id: int, alt_id: int):
