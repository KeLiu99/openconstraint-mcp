...
- **`run_cpsat_python_experiment(attempts, objective_sense=None, …)`** — run a list
  ...
  **Reproducibility:** an experiment winner reflects **one observed run**,
  ...
  no formulation wins outright — optional intervals give the best mk15
  incumbent, redundant bounds give the only nontrivial lower bound at 60
  machines, and on the largest instance all three are beaten by a greedy
