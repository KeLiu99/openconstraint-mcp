...
- **`examples/social_golfers`** — the Social Golfer Problem: schedule `n_groups`
  ...
  `cpsat_24/` is a third, `feasible`-only construction for the 8-3-11 instance,
  which cannot cover every pair (12 of `C(24,2) = 276` pairs are necessarily
  missed); see its `problem.txt` for the class-based construction that bounds
  the miss count.
...
- **`examples/nonogram`** — a 5x5 nonogram puzzle (`solve satisfy`): shade
  ...
  `examples/nonogram/python/` is a `checked`-level `save_verified_cpsat_python`
  bundle for the same puzzle — `solution.py`, `checker.py`, `problem.txt`,
  `solution.json`, and `.openconstraint-model.json` — the CP-SAT analogue of
  the MiniZinc save above.
...
| `examples/nonogram/python` | general CSP | checked save (CP-SAT analogue of the MiniZinc save above) | `save_verified_cpsat_python` | `test_examples_manifest.py` (manifest integrity only; no dedicated integration test) | none |
...
| `examples/social_golfers/cpsat_24` | scheduling/rostering | CP-SAT direct solve, `feasible`-only (no checker) | `run_cpsat_python` | `test_examples_manifest.py` (manifest integrity only; no dedicated solve/integration test) | reported-only save, older manifest schema (no `level`/`checker`); not superseded by a checked save the way `cpsat/` is by `cpsat_best/` |
