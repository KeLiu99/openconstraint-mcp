...
| `examples/nurse_rostering` | scheduling/rostering | checker-backed CP-SAT solve against a published benchmark optimum, with an independent scorer as the checker | `run_cpsat_python_file`, `run_cpsat_python_file_checked` | `tests/examples/test_nurse_rostering.py` | the real hospital instance (QMC-2 from schedulingbenchmarks.org, 19 nurses x 28 days, all 121 rules soft) reaches the published optimum of 29 but needs `--time-limit 300`; `verify_model.py`'s third gate re-solves it on four seeds and runs for several minutes, so only its first two gates are in `just check` |
