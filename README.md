...
| `examples/online_printing_shop` | scheduling/optimization | checker-backed CP-SAT solve with resumable operations, machine calendars, and sequence-dependent setups | `run_cpsat_python_file`, `run_cpsat_python_file_checked` | `tests/examples/test_online_printing_shop.py` | only `data_sops1.json` proves optimality quickly (<1s); `data_mops1.json` and `data_lops.json` need `config={"max_time_in_seconds": N}` (set below `timeout_ms`) so CP-SAT stops itself and returns a checkable incumbent — without it the child is killed at `timeout_ms` and the checker refuses the resulting `status="timeout"` |
