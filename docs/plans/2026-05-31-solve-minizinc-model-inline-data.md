...
## Implementation deviation (recorded post-build)

**The shipped command shape is the positional `<model>.mzn <data>.dzn` form, not the `--data` flag.** During Task 5 (real-runtime integration), the bundled binary was found to reject `--data`/`-d` for the findMUS meta-solver (`org.minizinc.findmus`) — it re-parses the command line with a restricted CLI that only accepts a *positional* data file — while `cp-sat` and the `-c` compile accept both. The positional order (MiniZinc's documented `model.mzn data.dzn`) is honored by all three paths and was adopted with the user's approval.

Consequently, **Decision 1 below and every `--data` / "model file stays last" reference in this plan are superseded**: the data file is appended *after* the model file, so `cmd[-1]` is the `.dzn` when data is present (`cmd[-2]` is the `.mzn`), and the command-shape tests assert `"--data" not in cmd`. The Task-5 findMUS smoke model was likewise changed from `x >= lo` / `x <= hi` to a sum-based form (`x + y > lo` / `x + y < hi`, `x != y`): a direct single-variable bound contradiction is detected at flatten time and folded into findMUS's hard "background", yielding no soft-constraint MUS. Everything else in the plan (the three-tool scope, `data is not None` gate, shared isolation, model-only structured core, schema/runtime no-touch) shipped as written.
...
`solve_minizinc_model`, `check_minizinc_model`, and `find_unsat_core` accept an optional `data: str | None` of inline MiniZinc data. When provided, it is written to `data.dzn` in the model's temp dir and passed to the managed `minizinc` binary via `--data <path>`. When omitted (`None`), behavior is byte-identical to today.
...
- `_run_managed_minizinc` already owns the temp dir and command construction, so the data file must be written there. It gains a `data: str | None = None` parameter. When `data is not None`, it writes `data.dzn` (UTF-8) into the same `TemporaryDirectory` and adds `--data <data_path>` to argv. The model file remains the **last** argument.
...
1. **Pass data via the documented `--data` flag; model file stays last.** Official MiniZinc docs document `-d, --data <file>` ("File named `<file>` contains data used by the model") and show the canonical positional order as model-before-data (`minizinc --solver gecode model.mzn data.dzn`). Using `--data <data_path>` keeps the model file as `cmd[-1]`, which preserves `_run_managed_minizinc`'s documented "model file is always the last command argument" invariant and every existing command-shape test/recorder (which read `cmd[-1]`). We do **not** reorder the positional args to `data.dzn model.mzn` — that is contrary to the documented order and was only ever a workaround for the test recorder.
2. **`data is not None` is the gate.** `None` = omitted (no data file written, no `--data` flag, command byte-identical to today). A provided string — including `""` — is written verbatim to `data.dzn` and passed via `--data`; an empty data file is harmless to MiniZinc. We do **not** strip-validate or reject empty data (unlike `model`, which must be non-empty), because empty/whitespace data is a meaningful "no parameters" input, not an error.
...
| ---- | ------ | -------------- |
| `src/openconstraint_mcp/minizinc.py` | Modify | Add `_DATA_FILENAME = "data.dzn"`; add `data: str | None = None` to `_run_managed_minizinc` (write + `--data <path>` when not `None`); add `data: str | None = None` to `solve_model`, `check_model`, and `find_unsat_core` and forward it. `_parse_unsat_core` unchanged. |
...
| `tests/test_minizinc.py` | Modify | Add `solve_model`, `check_model`, and `find_unsat_core` data tests: data provided → `--data` present, data path follows it, `data.dzn` contents match, model file is still `cmd[-1]`; data omitted → no `--data`, no `.dzn`. For `find_unsat_core`, also assert the structured core still resolves from the model text. |
...
def _run_managed_minizinc(
    ...
    *,
    ...
    data: str | None = None,   # written to data.dzn, passed via --data; model file stays last
) -> _RunOutcome: ...
...
`[str(binary), "--solver", solver, "--time-limit", str(timeout_ms), *extra_args, *("--data", <data.dzn>) if data is not None else (), <model.mzn>]`
— so the model file remains the last argument whether or not data is present. (`find_unsat_core` uses `solver=FINDMUS_SOLVER`, `extra_args=()`, exactly as today.)
...
- [ ] Add `data: str | None = None` to `_run_managed_minizinc`. Inside the `TemporaryDirectory` block, when `data is not None`, write `data` to `tmp_dir / _DATA_FILENAME` (UTF-8) and add `"--data", str(data_file)` to the command **before** `str(model_file)` (model file stays last). When `data is None`, build the command exactly as today.
- [ ] Add `data: str | None = None` to `solve_model` and `check_model` and forward it to the helper.

Tests (monkeypatch `openconstraint_mcp.minizinc.subprocess.run`; read files at call time, since the temp dir is deleted on return):

- [ ] **solve: data via `--data`, model last:** `solve_model(model, data="n = 3;")` → `--data` in argv, the arg after it ends `.dzn` and its contents equal `"n = 3;"`; `cmd[-1]` is the `.mzn`. Mocked `rc=0`, `stdout="==========\n"` → `status="optimal"`.
- [ ] **solve: no data → no `--data`, no `.dzn`.**
- [ ] **check: data via `--data`, model last** (`-c` still present); mocked `rc=0` → `status="ok"`.
- [ ] **check: no data → no `--data`.**
...
- [ ] **data via `--data`, model last:** with the existing `_UNSAT_CORE_MODEL` (extend it or use a parameterized variant) and `data=…`, recorded argv has `--data` followed by a `.dzn` whose contents match; `cmd[-1]` is the `.mzn`; `solver` is `FINDMUS_SOLVER`; `-c` absent. With the mocked `_UNSAT_CORE_STDOUT`, `status="mus_found"` and the structured `core` still resolves from the model text (model-only spans, per Decision 6).
- [ ] **no data → no `--data`.**
...
- [ ] Add a single concise sentence to `_SOLVE_CONSTRAINT_PROBLEM_PROMPT`: when the drafted model relies on inline data, pass the **same** `data` to both `check_minizinc_model` and `solve_minizinc_model`. Do **not** rewrite the prompt or alter the existing recommended-loop line (the ordering test anchors on it).
...
- [ ] Add a `data: str | None = None` bullet to the argument lists of `solve_minizinc_model`, `check_minizinc_model`, and `find_unsat_core`: optional inline MiniZinc data (`.dzn` contents — any data assignments, not parameter-only) provided directly as text; omit (or pass `null`) for models that need no external data; written to a private temp file alongside the model and passed to the managed runtime via `--data` — never a client-supplied path.
...
### Task 5: Real-runtime integration smoke for `--data`

**Files:** modify `tests/test_minizinc_integration.py`.

The mocked unit tests prove argv shape but **cannot** prove the bundled runtime honors `--data` — the plan's top risk. Add real-binary smokes (marked `integration` via the file's existing `pytestmark`, gated by the existing `_require_runtime` autouse skip), one per solver path that takes data. Use a parameterized model so a missing/ignored `--data` would *fail* rather than pass — making the test a genuine proof the flag is honored.

- [ ] **solve honors `--data` (cp-sat):** `solve_model("int: n;\nvar 1..n: x;\nconstraint x = n;\nsolve satisfy;\noutput [\"x=\\(x)\\n\"];", data="n = 4;")` → `status in {"satisfied", "optimal"}` and `"x=4" in result.stdout` (the data forces `x` to `4`).
- [ ] **check honors `--data` (-c):** `check_model(<same parameterized model>, data="n = 4;")` → `status == "ok"` (without data this model fails to flatten — the unassigned `n` bounds a variable domain (`var 1..n: x`), so a clean `ok` proves the data was read).
- [ ] **find_unsat_core honors `--data` (findMUS):** with `model="int: lo;\nint: hi;\nvar 0..10: x;\nconstraint x >= lo;\nconstraint x <= hi;\nsolve satisfy;"` and `data="lo = 5;\nhi = 3;"` → `status == "mus_found"`, and the normalized core sources contain `x >= lo` and `x <= hi` (proves the data was read *and* that the model-only core resolves correctly with data present).
...
- With `data` provided, argv contains `--data` followed by a path ending `.dzn` (contents == the provided text), and the model `.mzn` is still `cmd[-1]`; tests pin this for all three tools.
- With `data` omitted, the command is byte-identical to today (no `--data`, no `.dzn` written); the full pre-existing suites pass unchanged.
...
- Real-runtime integration smokes (`tests/test_minizinc_integration.py`, `integration`-marked) prove the bundled binary honors `--data` for the cp-sat, `-c`, and `org.minizinc.findmus` paths via parameterized models; `just integration` is green where a runtime is installed.
...
- **`--data` flag correctness.** The plan relies on `-d/--data <file>` being honored by the bundled runtime for the cp-sat, `-c`, and findMUS paths. Verified against official MiniZinc command-line docs; the **real-binary confirmation is Task 5's `integration` smokes** (parameterized models where ignored data would fail, not pass), run via `just integration`. The mocked unit tests only pin argv shape, not runtime acceptance.
- **Empty-string data.** Passing `data=""` writes an empty `data.dzn` and still passes `--data`. Harmless (no parameters assigned) and intentional per Decision 2; documented as "`None` means omitted".
