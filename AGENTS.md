...
- **Check `shared/` (and sibling leaves) for an existing helper before writing a new one.** A quick grep beats a duplicate — `shared/save_target.py` and `shared/job_errors.py` already hold the hashing, time, and error-summary primitives multiple modules need.
