...
# ---------------------------------------------------------------------------
# CP-SAT Python seed-sweep schemas
# ---------------------------------------------------------------------------

# A seed sweep's overall outcome. ``winner`` ⇔ an accepted attempt was selected
# (its full ``CpsatPythonResult`` plus ``winner_index``/``winner_seed`` are set);
# the winner's own ``status`` stays authoritative — a sweep never proves optimality.
# ``no_winner`` ⇔ no attempt was accepted. This ⇔ is enforced below.
CpsatSweepStatus = Literal["winner", "no_winner"]

# How a sweep winner is chosen, surfaced as a typed (not free-form) value so the
# client gets an enumerable contract: best objective for the requested sense, then
# stronger status (optimal > feasible > timeout), then earliest provided seed order.
CpsatSweepSelectionPolicy = Literal["best_objective_then_status_then_seed"]

# A per-attempt checker verdict (None when no checker was supplied for the sweep,
# or when the attempt failed base acceptance and the checker was never run).
CpsatSweepCheckerStatus = Literal["accepted", "rejected", "error", "timeout"]


class CpsatPythonSweepAttempt(BaseModel):
    """One seed's attempt in a CP-SAT Python seed sweep and its observed outcome.

    Records every attempted seed — accepted, rejected, and failed alike — so the
    full sweep is visible without re-running. ``accepted`` is the server's verdict
    after base acceptance (status ∈ {optimal, feasible, timeout}, a non-empty
    solution, a finite numeric objective) and, when a checker was supplied, the
    checker gate. ``checker_status`` is ``None`` when no checker ran for this
    attempt (no checker supplied, or the attempt already failed base acceptance,
    so the checker was never spent on it). ``objective`` is the attempt's reported
    objective (or ``None`` when non-numeric/absent); ``message`` carries a short
    reason when the attempt was not accepted.
    """
    ...
    objective: float | int | None
    accepted: bool
    checker_status: CpsatSweepCheckerStatus | None = None
    message: str | None = None
    timed_out: bool
    truncated: bool
    duration_ms: int
...
class CpsatPythonSweepResult(BaseModel):
    """Outcome of a CP-SAT Python seed sweep: the best accepted result and the table.

    ``status="winner"`` carries the winning attempt's ``winner_index`` (a 0-based
    handle into ``attempts``), ``winner_seed``, and the full ``winner``
    ``CpsatPythonResult``; ``status="no_winner"`` leaves all three ``None`` because
    no attempt was accepted. The invariant ``winner ⇔ winner_index ⇔ winner_seed ⇔
    status == "winner"`` is enforced (all present together or all absent), and when
    present ``winner_index`` is a valid index into ``attempts`` whose ``seed``
    equals ``winner_seed`` — so a client can branch on ``status`` and trust the
    winner fields without re-scanning by seed.

    A ``timeout`` winner is a REPORTABLE best incumbent, not a SAVABLE one: it
    fails ``save_verified_cpsat_python``'s reported gate (which requires
    ``optimal``/``feasible``). The winner's own ``status`` remains authoritative —
    a better incumbent from a sweep is not a new proof of optimality.

    ``distinct_accepted_objectives`` counts unique objective values among accepted
    attempts. ``seed_variation_hint`` is present only when the sweep engine saw
    ...
    attempts: list[CpsatPythonSweepAttempt] = Field(default_factory=list)
    elapsed_ms: int
    objective_sense: CpsatObjectiveSense
    ...
    def _winner_presence_matches_status(self) -> CpsatPythonSweepResult:
        has_winner = self.winner is not None
        index_present = self.winner_index is not None
        seed_present = self.winner_seed is not None
        status_winner = self.status == "winner"
        if not (has_winner == index_present == seed_present == status_winner):
            raise ValueError(
                "CpsatPythonSweepResult requires winner, winner_index, winner_seed, "
                "and status=='winner' to agree (all present together or all absent)"
            )
    ...
    def _validate_winner_attempt_fields(self) -> None:
        ...
        if self.attempts[self.winner_index].seed != self.winner_seed:
            raise ValueError(
                "winner_seed must equal attempts[winner_index].seed "
                f"({self.winner_seed} != {self.attempts[self.winner_index].seed})"
            )
