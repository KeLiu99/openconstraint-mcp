...
SAVE_VERIFIED_CPSAT_PYTHON_DESCRIPTION = (
    ...
    "checker or expectation without writing to a directory you would discard. It "
    "runs the SAME solver child and the SAME gates in the SAME order, skipping only "
    ...
    "`OPENCONSTRAINT_MCP_CPSAT_CONFIG`, so a cooperating script picks both up. The "
    "seed is recorded in the manifest and `config` is persisted as "
    "`replay-config.json`; the saved `model.py` is byte-for-byte the submitted "
    "script and carries only its own seed fallback. "
    "Optional `experiment_result` (a CpsatPythonExperimentResult from "
    "`run_cpsat_python_experiment`) is PROVENANCE ONLY — never verification "
    "evidence; the fresh re-run and gates decide everything. When supplied it must "
    "be self-consistent with this save request or the save is REJECTED before any "
    "child runs: `status=='winner'`, and at least one ACCEPTED attempt in "
    ...
    "successful save the full attempt table is written as `experiment-log.json`, a "
    "provenance SUMMARY (per-attempt hashes and scalar outcomes) that never archives "
    "a non-saved attempt's `config`. "
    "Saved seed/config provenance improves replayability but guarantees no "
    "bit-for-bit reproducibility: CP-SAT randomness, parallel search, and "
    "script-level nondeterminism can still yield a different (but still valid) "
    "solution, and the gates check status, not solution-equality. This fresh "
    "save-time run is the authority. "
