...
RUN_CPSAT_PYTHON_EXPERIMENT_DESCRIPTION = (
    ...
    "`objective_sense` ('maximize'|'minimize'). Optional: `default_timeout_ms` "
    ...
    "oversubscribing the machine can make runs slower and less stable. Results "
    ...
    "`optimal`/`feasible`/`timeout`, a non-empty solution, and a finite numeric "
    "objective), then — only for base-eligible attempts — the optional checker "
    "gate (accepted iff the checker returns `accepted`). The winner is the "
    "accepted attempt with the best objective for `objective_sense`, ties "
    "broken by stronger status (optimal > feasible > timeout) then earliest "
    "attempt order. "
    ...
    "`objective_sense`, `selection_policy`, `source_sha256` (index-aligned with "
    "`attempts`), `checker_sha256`, `problem_sha256`. A `timeout` winner is "
...
SAVE_VERIFIED_CPSAT_PYTHON_DESCRIPTION = (
    ...
    "(`status=='winner'`, the winning attempt's `source_sha256` matching `source`, "
    "its `seed` matching the supplied `seed`, its `config_sha256` matching the "
    "canonical hash of the supplied `config`) or the save is REJECTED before any "
    "child runs; the fresh re-run and gates below still decide everything. On a "
    "successful save its full attempt table is written as `experiment-log.json` — "
