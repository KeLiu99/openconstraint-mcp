...
SAVE_VERIFIED_MINIZINC_MODEL_DESCRIPTION = (
    ...
    "unseeded winner matches an unseeded save); or the winning formulation's/"
    "data's hash does not match `model`/`data`. A `checker_sha256` mismatch is "
    ...
    "objective, timing) plus a compact summary under the manifest's "
...
RUN_CPSAT_PYTHON_SWEEP_DESCRIPTION = (
    "Run ONE LLM-generated OR-Tools CP-SAT Python `source` multiple times under "
    "different solver random seeds and return the best ACCEPTED result plus a "
    "compact attempt table. Use when an optimization model can find different "
    "incumbents under different seeds within a fixed per-run budget. This is a "
    "QUALITY improvement across runs, NOT a new optimality proof — the winner's "
    "own `status` stays authoritative. "
    "Seed protocol: each attempt re-runs `source` with the environment variable "
    "`OPENCONSTRAINT_MCP_CPSAT_SEED` set to that seed. The script MUST read it and "
    "assign `solver.parameters.random_seed` (the server cannot force a seed into "
    "arbitrary Python); a non-cooperating script ignores the sweep. Prefer "
    "`solver.parameters.num_workers = 1` for reproducible seed sweeps. "
    "Attempts run SERIALLY in the given `seeds` order. "
    "Required args: `source`, `seeds` (a non-empty list of unique non-bool "
    "integers in the CP-SAT random_seed signed-int32 range), and "
    "`objective_sense` ('maximize'|'minimize', needed to compare "
    "numeric objectives). Optional: `per_run_timeout_ms` (per-attempt cap, "
    "defaults to the run_cpsat_python default), `problem` (forwarded to the "
    "checker payload), `checker` (a Python checker source string), and "
    "`checker_timeout_ms` (defaults to `per_run_timeout_ms`). "
    "Acceptance is two ordered gates: base acceptance (status `optimal`/`feasible`/"
    "`timeout`, a non-empty solution, and a finite numeric objective), then — only "
    "for base-eligible attempts — the optional checker gate (accepted iff the "
    "checker returns `accepted`). An attempt that fails base acceptance never "
    "spends a checker run. The winner is the accepted attempt with the best "
    "objective for `objective_sense`, ties broken by stronger status "
    "(optimal > feasible > timeout) then earliest provided seed order (list "
    "position, not numeric seed value). "
    "BUDGET GATE: the sweep is synchronous, so it is rejected UP FRONT (before any "
    "child runs) when its projected wall-clock budget — per-seed run (and checker) "
    "budgets plus a conservative per-child timeout/kill overhead, summed over the "
    "seeds — exceeds a fixed cap, and separately when the seed count exceeds "
    "8. These two gates are independent: at the default per_run_timeout_ms, the "
    "wall-clock budget binds well before 8 seeds (only a handful fit). Reaching "
    "the full 8-seed cap requires a correspondingly smaller per_run_timeout_ms "
    "(and checker_timeout_ms, if checking — checked sweeps cost roughly twice as "
    "much per seed). "
    "Returns a CpsatPythonSweepResult: `status` ('winner'|'no_winner'), "
    "`winner_index`/`winner_seed`/`winner` (a full CpsatPythonResult, all present "
    "iff 'winner'), `attempts` (every seed, accepted or not), `elapsed_ms`, "
    "`objective_sense`, `selection_policy`, `distinct_accepted_objectives`, and "
    "`seed_variation_hint` (nullable, set only for identical accepted incumbents "
    "that ran to completion — optimal/feasible — as a conditional seed-handling "
    "check). "
    "A `timeout` winner is a REPORTABLE best incumbent but NOT savable: "
    "`save_verified_cpsat_python` requires `optimal`/`feasible`, so re-run the "
    "winning seed with a larger `per_run_timeout_ms` until it reports "
    "`optimal`/`feasible`, then save it (passing that `seed`). " + _CPSAT_CHILD_POSTURE
)
...
SAVE_VERIFIED_CPSAT_PYTHON_DESCRIPTION = (
    ...
    "range) replays a specific CP-SAT random seed: "
    "the re-run sets `OPENCONSTRAINT_MCP_CPSAT_SEED` so a cooperating script uses "
    "that seed (e.g. to persist a `run_cpsat_python_sweep` winner), and the seed is "
    "recorded in the manifest. The save gates are UNCHANGED — the reported gate "
    "still requires `optimal`/`feasible`, so a `timeout` sweep winner is not "
    "savable even with its seed replayed. To reproduce a seeded save by hand, set "
    "`OPENCONSTRAINT_MCP_CPSAT_SEED` to the recorded seed; the saved `solution.py` "
    "is byte-for-byte the script and carries only its own seed fallback. "
    ...
    "mismatch is NOT rejected — it only affects what the persisted log "
    "records. When supplied and every gate passes, `experiment-log.json` is "
    "written (role `experiment_log` in `files`) recording every sweep attempt "
    "(seed, status, objective, accepted, checker status, message, timing) plus "
    "a compact summary under the manifest's `verification`. "
    ...
    "`experiment-log.json` only when `sweep_result` is supplied and the save "
    "succeeds; and a `.openconstraint-model.json` manifest recording tool "
    "version, timestamp, verification level, expectation settings, checker "
    "summary (status/error_count/duration/timed_out/truncated only — no free "
    "text), and per-file sha256 hashes. "
