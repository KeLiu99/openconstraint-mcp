...
MCP_SERVER_INSTRUCTIONS = (
    ...
    "LLM-fluent; suits pure integer/scheduling problems, imperative "
    "preprocessing, and custom data structures.\n"
    "- MiniZinc: expressive global constraints, `.dzn` data files, checker "
    ...
    "within one or both backends and presents only the full-instance final "
    "result.\n"
    "\n"
    "WITHOUT PROMPTS:\n"
    "- MiniZinc: draft the model in the client LLM, check it with "
    "check_minizinc_model, then solve with solve_minizinc_model. Use "
    "inspect_minizinc_model to learn which parameters a model needs as data "
    "(plus its output variables and solve method) before building a `.dzn`. "
    "When the model and data already exist as local files, pass their paths "
    "to check_minizinc_files / solve_minizinc_files instead of pasting "
    ...
    "\n"
    "SAVING: when the user asks to save a verified result, persist it with "
    ...
    "opens a file dialog.\n"
    "\n"
    "PRESENTING RESULTS — lead with the result: a plain-language status, the "
    "solution stated in the terms of the user's problem (not the raw JSON "
    "result), a compact item table when the problem supplies item-like data, "
    "and the complete model-visible `Statistics:` section when present — do "
    "not condense it to selected fields.\n"
    "\n"
...
MCP_SERVER_INSTRUCTIONS_CORE = (
    ...
    "LLM-fluent; suits pure integer/scheduling problems, imperative "
    "preprocessing, and custom data structures.\n"
    "- MiniZinc: expressive global constraints and `.dzn` data files, run "
    "through the managed local runtime.\n"
    "\n"
    "WORKFLOW:\n"
    "- MiniZinc: draft the model in the client LLM, check it with "
    ...
    "`include` resolves. Use check_runtime to confirm the managed MiniZinc "
    "runtime is installed and list_available_solvers to enumerate its "
    "solvers.\n"
    "- CP-SAT: write a conforming script and run it with run_cpsat_python "
    "(bounded child process: timeout, 1 MB output cap, tree-kill), or "
    "run_cpsat_python_file for a script already on disk.\n"
    "\n"
    "PRESENTING RESULTS — lead with the result: a plain-language status, the "
    "solution stated in the terms of the user's problem (not the raw JSON "
    "result), a compact item table when the problem supplies item-like data, "
...
LIST_AVAILABLE_SOLVERS_DESCRIPTION = (
    ...
    "two cases distinct: (a) `std_flags` can list flags with no named control at "
    ...
    "text content is a complete `id`/`name`/`version` inventory table of every "
    "solver: copy it into the final answer verbatim — do not omit rows, convert "
    "to bullets/prose, summarize, or group. It ends with a user-visible note "
    "(the SolverList's `capability_note`) that detailed capabilities are "
    "available on request. The full `capabilities` metadata (`supports_*` "
    "booleans and `std_flags`) lives only in the structured result and "
    "is not printed by default — surface it when the user asks."
)
...
_SOLVE_MINIZINC_MODEL_BODY = (
    ...
    "summarizing selected fields. Optional solver/search controls (an omitted "
...
RUN_CPSAT_PYTHON_FILE_CHECKED_DESCRIPTION = (
    ...
    "is skipped. "
    "`test_checker` (OPT-IN, default false) tests the CHECKER, not the model: "
    "after an `accepted` verdict — and only then — the same checker is re-run "
    "against four deterministic, domain-agnostic mutations of the solution "
    "(perturbed objective, dropped element, duplicated element, perturbed "
    "numeric field). The numeric mutation prefers an integer, then flips a "
    "boolean when no integer exists, so flat boolean assignments remain "
    "probeable. `checker_test` carries the `baseline` verdict, one "
    "`mutations` row per mutation, and two counts: `rejected_count` (graded "
    "and refused — the checker is not vacuous) and `accepted_count` (graded and "
    "swallowed — the vacuous-checker signal). An `error`/`timeout` mutant "
    "reached no verdict and counts in neither, same as a mutation that could "
    "not apply or whose probe faulted (`skipped_reason`) — so "
    "`rejected_count: 0, accepted_count: 0` alone cannot tell \"nothing was "
    "corruptible\" from \"every mutant errored out\"; read `mutations` "
    "directly for that. A positive "
    "`rejected_count` is evidence of NON-VACUITY, NOT COMPLETENESS — never that "
    "the checker grades every constraint, and no checker proves optimality. "
    "These mutations are not known-invalid, so `rejected_count: 0` is "
    "inconclusive and raises no top-level diagnostic; for stronger assurance use "
    "a problem-specific, known-invalid payload. `checker_test` is null when the "
    "baseline was not `accepted` — there is nothing to test the checker against. "
    "WALL CLOCK: nominally `(timeout_ms + ~8s) + (checker_timeout_ms + ~8s)`, "
    "plus `(applied mutations) * (checker_timeout_ms + ~8s)` when `test_checker` "
    "is on — ~76.5 s at the 30 s defaults without self-testing; an explicitly "
    "requested 30 s checker timeout with all four mutations would project to "
    "~229.5 s and is rejected. `test_checker` is GATED and nothing else is: "
    "with it on, the projected worst case is limited to 120 s, charging every "
    "child its timeout plus conservative termination/poll overhead and assuming "
    "all four mutations apply. When `checker_timeout_ms` is omitted, the server "
    "uses the smaller of `timeout_ms` and the largest fitting checker budget; "
    "the 30 s model default therefore derives 8100 ms and fits exactly. An "
    "explicit over-budget value is rejected before any child runs, as is an "
    "implicit request whose model timeout leaves no positive checker budget. "
    "Without `test_checker`, `timeout_ms` has no upper bound: "
    "set your client's tool timeout accordingly. For a solve longer than a "
    ...
    "and it offers no `test_checker`: the self-test is synchronous-only. "
...
RUN_CPSAT_PYTHON_EXPERIMENT_DESCRIPTION = (
    ...
    "`problem_sha256`, `warnings` (advisory strings: a reproducibility "
    "disclaimer whenever there is a winner, plus the num_workers-"
    "oversubscription warning when triggered; empty otherwise). "
...
SAVE_VERIFIED_CPSAT_PYTHON_DESCRIPTION = (
    ...
    "saved `model.py` is byte-for-byte the script and carries only its own "
    "seed fallback. "
