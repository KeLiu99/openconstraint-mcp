...
RUN_CPSAT_PYTHON_FILE_CHECKED_DESCRIPTION = (
    ...
    "is on — ~76.5 s at the 30 s defaults, ~229.5 s with all four mutations. "
    "`test_checker` is GATED and nothing else is: with it on, a projected worst "
    "case over 120 s is rejected before any child runs, charging every child its "
    "timeout plus conservative termination/poll overhead and assuming all four "
    "mutations apply — so the 30 s defaults do NOT fit and `test_checker` needs a "
    "smaller `checker_timeout_ms` (`timeout_ms=30000, checker_timeout_ms=8000` "
    "fits at ~119.5 s). Without `test_checker`, `timeout_ms` has no upper bound: "
