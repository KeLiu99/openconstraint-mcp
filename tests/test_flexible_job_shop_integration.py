...
between two separately-tested artifacts -- ``model.py``'s printed `solution`
...
``test_compact_summary_solution_yields_error_status`` was written for.
...
# The proven optimum of the 10x6 Brandimarte mk01 instance. model.py reaches it
# in ~0.1s single-worker at seed 42, so the 10s in-model cap below is a wide
# margin rather than a race -- the assertion pins a property of the INSTANCE,
# not a solver-performance timing.
_MK01_OPTIMUM = 40


@pytest.mark.integration
@pytest.mark.asyncio
async def test_mk01_model_and_checker_reach_an_accepted_verdict_through_the_mcp_tool() -> None:
