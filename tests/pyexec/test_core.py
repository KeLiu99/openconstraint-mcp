...
def test_a_faulted_mutation_probe_keeps_the_verdicts_already_earned(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # The fault is raised building the mutant, OUTSIDE `_run_checker_or_report`'s
    # reach — that guard would absorb it into an `error` report — so this
    # exercises the per-mutation guard. The loop must record the faulted row and
    # carry on, not discard the rejections the other mutations earned.
    built = 0
    ...
    assert result.checker_test is not None
    # Row 1 (`objective_perturbed`) leaves the solution untouched, so this stub
    # accepts it; row 2 faults; rows 3 and 4 mutate the solution and are
    # rejected. Without the per-mutation guard the fault would reach the outer
    # one and `checker_test` would be None, losing both rejections.
    assert result.checker_test.rejected_count == 2
    assert [m.report is not None for m in result.checker_test.mutations] == [
        True,
        False,
        True,
        True,
...
def test_a_faulted_mutation_probe_reports_its_reason(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    built = 0

    class _FaultsOnTheFirstMutant(CpsatPythonResult):
        def model_copy(self, **kwargs: Any) -> CpsatPythonResult:
            nonlocal built
            built += 1
            if built == 1:
    ...
    _patch_checker_test(
        monkeypatch,
        _FaultsOnTheFirstMutant(**base.model_dump()),
        _accept_everything,
    )
    ...
    assert result.checker_test is not None
    faulted = result.checker_test.mutations[0]
    assert faulted.skipped_reason == "mutation probe failed: RuntimeError: mutant build exploded"


def test_a_faulted_mutation_probe_does_not_void_the_run(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    class _AlwaysFaults(CpsatPythonResult):
        def model_copy(self, **kwargs: Any) -> CpsatPythonResult:
            raise RuntimeError("mutant build exploded")
    ...
    assert result.status == "optimal"
    assert result.solution == _MUTABLE_SOLUTION
    assert result.checker is not None
    assert result.checker.status == "accepted"
