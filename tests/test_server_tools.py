...
def _fake_sweep_attempt(
    *,
    seed: int,
    status: str = "feasible",
    objective: float | int | None = 5,
    accepted: bool = True,
) -> Any:
    from openconstraint_mcp.schemas import CpsatPythonSweepAttempt
...
def _fake_sweep_result(*, status: str = "winner", attempts: Any = None, **kw: Any) -> Any:
    from openconstraint_mcp.schemas import CpsatPythonResult, CpsatPythonSweepResult

    attempts = attempts if attempts is not None else [_fake_sweep_attempt(seed=1)]
    common: dict[str, Any] = {
        "attempts": attempts,
        "elapsed_ms": 10,
        "objective_sense": "minimize",
    ...
    winner_index = kw.get("winner_index", 0)
    return CpsatPythonSweepResult(
        status="winner",
        winner_index=winner_index,
        winner_seed=attempts[winner_index].seed,
        winner=CpsatPythonResult(
            status=kw.get("winner_status", "feasible"),  # type: ignore[arg-type]
            solution={"x": 1},
            objective=kw.get("winner_objective", 5),
            stdout="",
            stderr="",
            return_code=0,
            timed_out=kw.get("winner_status") == "timeout",
            truncated=False,
            duration_ms=5,
        ),
        **common,
...
@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_is_listed() -> None:
    mcp = create_mcp_server()
    tools = await mcp.list_tools()
    assert "run_cpsat_python_sweep" in {t.name for t in tools}


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_schema_includes_params() -> None:
    mcp = create_mcp_server()
    tools = await mcp.list_tools()
    tool = next(t for t in tools if t.name == "run_cpsat_python_sweep")
    props = set(tool.inputSchema["properties"])
    assert {
        "source",
        "seeds",
    ...
    assert "ctx" not in props
    assert "tracker" not in props
    assert set(tool.inputSchema.get("required", [])) == {"source", "seeds", "objective_sense"}


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_routes_to_sweep(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seen: dict[str, Any] = {}

    def _fake(source: str, **kw: Any) -> Any:
        seen.update(kw)
        seen["source"] = source
        return _fake_sweep_result()

    monkeypatch.setattr("openconstraint_mcp.server.run_cpsat_python_sweep", _fake)
    mcp = create_mcp_server()
    result = _structured(
        await mcp.call_tool(
            "run_cpsat_python_sweep",
            {"source": "print('x')", "seeds": [1, 2], "objective_sense": "minimize"},
        )
    )
    assert result["status"] == "winner"
    assert seen["seeds"] == [1, 2]
    assert seen["objective_sense"] == "minimize"


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_notes_unsavable_timeout_winner(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "openconstraint_mcp.server.run_cpsat_python_sweep",
        lambda source, **kw: _fake_sweep_result(
            attempts=[_fake_sweep_attempt(seed=1, status="timeout")],
            winner_status="timeout",
        ),
    )
    mcp = create_mcp_server()
    text = _content_text(
        await mcp.call_tool(
            "run_cpsat_python_sweep",
            {"source": "print('x')", "seeds": [1], "objective_sense": "minimize"},
        )
    )
    assert "timeout" in text.lower()
    assert "not savable" in text.lower() or "not directly savable" in text.lower()


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_prints_engine_seed_variation_hint(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    attempts = [
        _fake_sweep_attempt(seed=1, status="feasible", objective=5),
        _fake_sweep_attempt(seed=2, status="feasible", objective=5),
    ]
    monkeypatch.setattr(
        "openconstraint_mcp.server.run_cpsat_python_sweep",
        lambda source, **kw: _fake_sweep_result(
            ...
            distinct_accepted_objectives=1,
            seed_variation_hint=(
                "Accepted feasible attempts produced the same objective and solution; "
                "if you expected seed diversity, confirm the script reads "
                "OPENCONSTRAINT_MCP_CPSAT_SEED and sets solver.parameters.random_seed."
            ),
        ),
    )
    mcp = create_mcp_server()
...
@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_does_not_infer_seed_hint_from_shared_objective(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Equal objective alone is common for alternate optima/feasible plateaus; the
    # sweep engine owns any hint and does not set one for this fake result.
    attempts = [
        _fake_sweep_attempt(seed=1, status="feasible", objective=5),
        _fake_sweep_attempt(seed=2, status="feasible", objective=5),
    ]
    monkeypatch.setattr(
        "openconstraint_mcp.server.run_cpsat_python_sweep",
        lambda source, **kw: _fake_sweep_result(attempts=attempts, distinct_accepted_objectives=1),
    )
    mcp = create_mcp_server()
    text = _content_text(
        await mcp.call_tool(
    ...
    assert "OPENCONSTRAINT_MCP_CPSAT_SEED" not in text


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_surfaces_validation_error() -> None:
    mcp = create_mcp_server()
    with pytest.raises(Exception, match="duplicates"):
        await mcp.call_tool(
            "run_cpsat_python_sweep",
            {"source": "print('x')", "seeds": [1, 1], "objective_sense": "minimize"},
        )


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_advertises_output_schema() -> None:
    # The tool returns CallToolResult, so it must be Annotated with the result model
    # for FastMCP to advertise/validate the promised outputSchema to listing clients.
    mcp = create_mcp_server()
    tools = await mcp.list_tools()
    tool = next(t for t in tools if t.name == "run_cpsat_python_sweep")
    assert tool.outputSchema is not None
    assert tool.outputSchema.get("title") == "CpsatPythonSweepResult"
    assert "attempts" in tool.outputSchema["properties"]


@pytest.mark.asyncio
async def test_run_cpsat_python_sweep_tool_rejects_bool_seed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # `seeds: list[StrictInt]` makes FastMCP/pydantic reject a JSON boolean at the
    # tool boundary instead of silently coercing `true` to int 1, enforcing the
    # documented "non-bool integers" contract on the MCP surface.
    called = False
    ...
    with pytest.raises(Exception, match="valid integer"):
        await mcp.call_tool(
            "run_cpsat_python_sweep",
            {"source": "print('x')", "seeds": [True, 2], "objective_sense": "minimize"},
        )
    assert not called
